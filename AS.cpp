...
void AS::poll(void) {
	...
	*/
	...
	if (snd.flag.active) snd.poll();																	// check if there is something to send
...
inline void AS::sendSliceList(void) {
	...
	if (snd.flag.active) return;																		// check if send function has a free slot, otherwise return
...
inline void AS::sendPeerMsg(void) {
	...
	} else if ((stcPeer.idx_cur) && (!snd.flag.timeout)) {											// peer index is >0, first round done and no timeout
...
void AS::preparePeerMessage(uint8_t *xPeer, uint8_t retries) {
	...
	snd.flag.max_retr = retries;																		// send only one time
}
...
void AS::processMessage(void) {
	...
	if        (rcv.msg.mBody.MSG_TYP == AS_MESSAGE_DEVINFO) {
		 ...
		 */
		if ((snd.flag.active) && (rcv.msg.mBody.MSG_CNT == snd.prev_msg_cnt) && (rcv.msg.mBody.BY10 != AS_RESPONSE_AES_CHALLANGE)) {
			snd.flag.retr_cnt = 0xFF;
		}
		...
		uint8_t authAck[4];
		//memcpy(authAck, rcv.buf + 10, 4);
		authAck[0] = rcv.buf[10];
		authAck[1] = rcv.buf[11];
		authAck[2] = rcv.buf[12];
		authAck[3] = rcv.buf[13];

		aes128_dec(rcv.buf+10, &ctx);														// decrypt payload with temporarily key again
...
//		rcv.prevBufUsed = 1;																		// ToDo: check if we need this here
	...
	inline void AS::processMessageResponseAES_Challenge(void) {
		...
		uint8_t challenge[6];
		memcpy(challenge, rcv.buf+11, 6);														// get challenge
...
void AS::prepareToSend(uint8_t mCounter, uint8_t mType, uint8_t *receiverAddr) {
	...
	snd.flag.active = 1;																				// remember to fire the message
}
